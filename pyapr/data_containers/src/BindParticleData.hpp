...
#pragma omp parallel for schedule(static) reduction(min : min_val)
     ...
     */
    template<typename ImageType>
    void sample_labels_with_boundary_py(APR &apr, py::array_t<ImageType, py::array::c_style> &img, int dilation) {
	...
	ParticleData<T> label_boundaries;
	this->sample_labels_with_boundary(apr, pd, label_boundaries, dilation, false);
        
        this->binary_map(label_boundaries, *this, [](const T a, const T b){return a+b;});
    }
...
void AddPyParticleData(pybind11::module &m, const std::string &aTypeString) {
    ...
    py::class_<TypeParticles, BaseParticles>(m, typeStr.c_str(), py::buffer_protocol())
            ...
            .def("sample_image", &TypeParticles::template sample_image_py<float>, "sample particle values from an image (numpy array)",
                 "apr"_a, "img"_a.noconvert())
            .def("sample_labels_with_boundary", &TypeParticles::template sample_labels_with_boundary_py<uint8_t>, "sample particle values from an image (numpy array)",
                 "apr"_a, "img"_a.noconvert(), "dilation"_a)
            .def("sample_labels_with_boundary", &TypeParticles::template sample_labels_with_boundary_py<uint16_t>, "sample particle values from an image (numpy array)",
                 "apr"_a, "img"_a.noconvert(), "dilation"_a)
            .def("sample_labels_with_boundary", &TypeParticles::template sample_labels_with_boundary_py<uint32_t>, "sample particle values from an image (numpy array)",
                 "apr"_a, "img"_a.noconvert(), "dilation"_a)
            .def("sample_labels_with_boundary", &TypeParticles::template sample_labels_with_boundary_py<uint64_t>, "sample particle values from an image (numpy array)",
                 "apr"_a, "img"_a.noconvert(), "dilation"_a)
            .def("sample_labels_with_boundary", &TypeParticles::template sample_labels_with_boundary_py<float>, "sample particle values from an image (numpy array)",
                 "apr"_a, "img"_a.noconvert(), "dilation"_a)
